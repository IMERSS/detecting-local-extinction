...
cracon_agm <- read.csv("Analysis_outputs/Intermediate/Crassula connata_accepted_grouped_merged.csv")
cracon_positive <- cracon_agm %>% dplyr::filter(search_effort > 0)
cracon_sf <- assign_cell_geometry_sf(cracon_positive, galgrid)
...
min_val <- min(cracon_sf$search_effort, na.rm = TRUE)
max_val <- max(cracon_sf$search_effort, na.rm = TRUE)
